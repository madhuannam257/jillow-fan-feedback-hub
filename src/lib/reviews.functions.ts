...
export const submitReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Fire-and-forget: notify the JILLOW CLUB inbox by email. This never
    // blocks or fails the review submission itself — if email delivery has
    // a problem, it's only logged server-side.
    const { sendReviewNotificationEmail } = await import("@/lib/email.server");
